...
class BaroonDealershipDashboard(models.TransientModel):
    ...
    availability_rate = fields.Float(readonly=True)
    reservation_rate = fields.Float(readonly=True)
    sold_rate = fields.Float(readonly=True)
    delivery_rate = fields.Float(readonly=True)
    ...
    def default_get(self, fields_list):
        ...
        portfolio_count = len(all_live_cars)

        availability_rate = self._safe_rate(len(available_cars), portfolio_count)
        reservation_rate = self._safe_rate(len(reserved_cars), portfolio_count)
        sold_rate = self._safe_rate(lot_model.search_count(sold_domain), portfolio_count)
        delivery_rate = self._safe_rate(lot_model.search_count(delivered_domain), portfolio_count)
    ...
    def _safe_rate(self, part, whole):
        if not whole:
            return 0.0
    ...
    def action_open_available(self):
        return self._open_window(_('Available Cars'), 'stock.lot', [('is_car_vehicle', '=', True), ('active', '=', True), ('car_status', '=', 'available')], context={'search_default_filter_car_vehicle': 1})

    def action_open_reserved(self):
        return self._open_window(_('Reserved Cars'), 'stock.lot', [('is_car_vehicle', '=', True), ('active', '=', True), ('car_status', '=', 'reserved')], context={'search_default_filter_car_vehicle': 1})

    def action_open_in_transit(self):
        return self._open_window(_('In Transit / Receiving'), 'stock.lot', [('is_car_vehicle', '=', True), ('active', '=', True), ('car_status', 'in', ['incoming', 'in_shipment', 'customs', 'received'])], context={'search_default_filter_car_vehicle': 1, 'group_by': 'car_status'})

    def action_open_preparation(self):
        return self._open_window(_('Preparation Queue'), 'stock.lot', [('is_car_vehicle', '=', True), ('active', '=', True), ('car_status', '=', 'in_preparation')], context={'search_default_filter_car_vehicle': 1})

    def action_open_sold(self):
        return self._open_window(_('Sold Cars'), 'stock.lot', [('is_car_vehicle', '=', True), ('car_status', '=', 'sold')], context={'search_default_filter_car_vehicle': 1, 'active_test': False})

    def action_open_delivered(self):
        return self._open_window(_('Delivered Cars'), 'stock.lot', [('is_car_vehicle', '=', True), ('car_status', '=', 'delivered')], context={'search_default_filter_car_vehicle': 1, 'active_test': False})

    def action_open_arriving_soon(self):
        return self._open_window(_('Arriving Soon'), 'stock.lot', [('is_car_vehicle', '=', True), ('active', '=', True), ('arriving_soon', '=', True)], context={'search_default_filter_car_vehicle': 1})
