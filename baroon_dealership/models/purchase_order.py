...
class PurchaseOrder(models.Model):
    ...
    def button_confirm(self):
        ...
        for order in self:
            ...
            incoming_pickings = order.picking_ids.filtered(lambda p: p.state not in ('done', 'cancel') and p.picking_type_id.code == 'incoming')
            if incoming_pickings:
                incoming_pickings._baroon_prepare_incoming_serials()
        return res
