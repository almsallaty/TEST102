...
class SaleOrderLine(models.Model):
    ...
    car_lot_id = fields.Many2one(
        ...
        domain="[('is_car_vehicle', '=', True), '|', ('car_status', '=', 'available'), '&', ('car_status', '=', 'reserved'), ('reservation_order_id', '=', order_id)]",
        copy=False,
    )
    ...
    def _prepare_car_line_description(self, lot):
        ...
        return ' • '.join(filter(None, [car_title, vin_part, engine_part]))
    ...
    def _onchange_product_id_clear_lot(self):
        require_vin = self.env['ir.config_parameter'].sudo().get_param('baroon_dealership.require_vin_on_sale', 'True') == 'True'
        for line in self:
            if line.car_lot_id and line.product_id and line.product_id != line.car_lot_id.product_id:
                line.car_lot_id = False
            if require_vin and line.product_id and line.product_id.product_tmpl_id.is_car_vehicle and not line.car_lot_id:
                line.name = line.name or _('Select the VIN before confirming this quotation.')

    @api.constrains('car_lot_id', 'product_uom_qty', 'product_id')
    def _check_car_line_quantity(self):
        ...
        for line in self:
            ...
            if require_vin and line.product_id and line.product_id.product_tmpl_id.is_car_vehicle and not line.car_lot_id and not line.display_type:
                raise ValidationError(_('Select a VIN / serial number for every car sale line.'))
