...
class StockLotDealershipExtension(models.Model):
    ...
    def _compute_extra_costs(self):
        for lot in self:
            ...
            for cost in (lot.landed_cost_ids | lot.expense_ids.mapped('landed_cost_id')).filtered(lambda c: c and c.state == 'done' and lot in c.car_lot_ids):
    ...
    def action_view_landed_costs(self):
        ...
        return {
            ...
            'context': {
                'default_car_lot_ids': [(6, 0, [self.id])],
            },
        }


    def action_print_car_movement(self):
        self.ensure_one()
        return self.env.ref('baroon_dealership.action_report_car_movement').report_action(self)

    def action_print_car_operations_pack(self):
        self.ensure_one()
        return self.env.ref('baroon_dealership.action_report_car_operations_pack').report_action(self)
