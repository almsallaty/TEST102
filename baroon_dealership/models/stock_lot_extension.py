...
class StockLotDealershipExtension(models.Model):
    ...
    @api.depends('expense_ids.amount', 'expense_ids.state', 'expense_ids.landed_cost_id')
    def _compute_extra_costs(self):
        for lot in self:
            ...
            lot.landed_cost_total = sum(approved.filtered(lambda e: e.landed_cost_id).mapped('amount'))

    @api.depends('vendor_bill_ids', 'landed_cost_ids', 'location_history_ids', 'audit_log_ids', 'test_drive_ids')
    def _compute_dealership_counts(self):
        for lot in self:
            vendor_bills = (lot.vendor_bill_ids | lot.expense_ids.mapped('account_move_id')).filtered(lambda m: m)
            ...
            lot.invoice_count = len(lot.sale_order_id.invoice_ids) if lot.sale_order_id else 0
    ...
    def action_view_vendor_bills(self):
        ...
        return {
            ...
            'view_mode': 'list,form',
            'domain': [('id', 'in', (self.vendor_bill_ids | self.expense_ids.mapped('account_move_id')).ids)],
        }
    ...
    def action_view_landed_costs(self):
        ...
        return {
            ...
            'view_mode': 'list,form',
            'domain': [('id', 'in', (self.landed_cost_ids | self.expense_ids.mapped('landed_cost_id')).ids)],
        }
