...
function getKitchenSentQty(line) {
    // `qty_sent` is the protected baseline already sent to the kitchen.
    // Extra quantity added after the last Send is still editable until sent again.
    const qtySent = parsePosNumber(line?.qty_sent, 0);
    if (qtySent > 0) {
        return qtySent;
    }

    // Fallback for restored/synced sent lines where qty_sent is not exposed.
    // Do not use dirty/change flags here, because those also mark new unsent edits.
    if (line?.sent_to_kitchen || line?.is_from_sync) {
        return parsePosNumber(line?.quantity, parsePosNumber(line?.qty, parsePosNumber(line?.get_quantity?.(), 0)));
    }
