...
function lineHasKitchenSentState(line) {
    return !!(
        line?.is_from_sync ||
        line?.sent_to_kitchen ||
        parsePosNumber(line?.qty_sent, 0) > 0 ||
        parsePosNumber(line?.quantity_sent, 0) > 0 ||
        parsePosNumber(line?.qty_printed, 0) > 0 ||
        parsePosNumber(line?.printed_qty, 0) > 0 ||
        line?.hasChange === false
    );
}
...
function getKitchenSentQty(line) {
    ...
    for (const value of explicitFields) {
        const qty = parsePosNumber(value, 0);
        if (qty > 0) {
            return qty;
        }
    }

    if (lineHasKitchenSentState(line)) {
...
function ensureSentQtySnapshot(line, oldQty) {
    if (!line || line._rwa_sent_qty > 0) {
        return;
    }
    if (lineHasKitchenSentState(line) && oldQty > 0) {
        line._rwa_sent_qty = oldQty;
    }
}
...
patch(Orderline.prototype, {
    async set_quantity(quantity, keepPrice) {
        ...
        // Important: snapshot the quantity that was already sent before allowing a new increase.
        // This lets waiters add new unsent qty and reduce only that unsent part before pressing Send again.
        ensureSentQtySnapshot(this, oldQty);
