...
function findLastPreparationLine(order, line) {
    if (!order || !line) {
        return null;
    }

    const lines = getPreparationLines(order);
    const directKeys = [line.preparationKey, line.uuid, line.uid, line.cid, line.id].filter(Boolean);
    ...
    for (const [key, value] of Object.entries(lines)) {
        ...
        if (directKeys.some((lineKey) => key === lineKey || key.startsWith(String(lineKey)))) {
            return value;
        }
        if (directKeys.includes(value.uuid) || directKeys.includes(value.line_uuid)) {
            return value;
        }
    }
...
function getSentQtyForLine(order, line) {
    ...
    const preparationLine = findLastPreparationLine(order, line);
    if (!preparationLine) {
        return 0;
    }

    const qty = Number(preparationLine.quantity ?? preparationLine.qty ?? 0);
